...
# Or export SUPABASE_URL / SUPABASE_ANON_KEY in your shell instead.
...
SUPABASE_ANON_KEY="${SUPABASE_ANON_KEY:-$INLINE_KEY}"
...
if [[ -z "$SUPABASE_URL" || -z "$SUPABASE_ANON_KEY" ]]; then
  ...
  echo "SUPABASE_URL and SUPABASE_ANON_KEY. Restore the project first if it was paused."
  exit 1
fi
