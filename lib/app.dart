...
class _LekoAppState extends ConsumerState<LekoApp> with WidgetsBindingObserver {
  ...
  /// Set when a user-change forced run was skipped because a run was in flight.
  bool _schedulerRetryAfterCurrent = false;
  ...
  void _maybeRunSchedulers({bool forceUser = false}) {
    if (_schedulerRunning) {
      if (forceUser) _schedulerRetryAfterCurrent = true;
      return;
    }
  ...
  Future<void> _runSchedulers(
    ...
    try {
      ...
      if (_schedulerRetryAfterCurrent) {
        _schedulerRetryAfterCurrent = false;
