...
class _LekoAppState extends ConsumerState<LekoApp> with WidgetsBindingObserver {
  ...
  /// Set when auth changes while a scheduler run is in flight.
  ...
  void _maybeRunSchedulers({bool forceUser = false}) {
    ...
    Future.microtask(
      () => _runSchedulers(ref, expectedUserId: userId, runDate: today),
    );
  }
  ...
  Future<void> _runSchedulers(
    ...
    var succeeded = false;
    try {
      if (ref.read(currentUserProvider)?.id != expectedUserId) return;
      ...
      if (succeeded) {
        _lastSchedulerRunDate = runDate;
        _lastSchedulerUserId = expectedUserId;
      }
