...
final dailyCloseoutsRepositoryProvider = Provider<DailyCloseoutsRepository>((
  ref,
) {
...
final closeoutsStreamProvider = StreamProvider<List<DailyCloseout>>((ref) {
  return ref
      .watch(dailyCloseoutsRepositoryProvider)
      .watchAllOrderedByDateDesc();
});

final streakProvider = FutureProvider<StreakResult>((ref) async {
  final settings = ref.watch(settingsStreamProvider).valueOrNull;
  if (settings == null) {
    return const StreakResult(currentStreak: 0, todayWithinBudget: false);
  }
